...
st.set_page_config(page_title="EDA - PhysioTrack", page_icon="🫀", layout="centered")
st.title("🫀 Analyse EDA")
st.caption("Activité électrodermale — réponse galvanique de la peau (GSR)")
...
# ── Détection des artefacts (chutes à 0 ou valeurs aberrantes) ─────────────────
# Seuil : valeur < 1% de la médiane du signal = artefact probable
seuil_artefact = max(np.median(signal) * 0.01, 1e-6)
artefact_mask = signal < seuil_artefact
n_artefacts = np.sum(artefact_mask)

if n_artefacts > 0:
    duree_artefacts = n_artefacts / sfreq
    st.warning(
        f"⚠️ **{n_artefacts} échantillons suspects détectés** ({duree_artefacts:.1f}s) "
        f"— probablement des déconnexions du capteur (valeurs proches de 0). "
        f"Ces segments sont marqués en rouge sur le graphe."
    )
    # Proposer correction par interpolation
    corriger = st.checkbox(
        "Corriger les artefacts par interpolation avant la décomposition",
        value=True,
        help="Remplace les valeurs aberrantes par interpolation linéaire. "
             "Recommandé pour améliorer la qualité de la décomposition."
    )
    if corriger:
        signal_clean = signal.copy().astype(float)
        signal_clean[artefact_mask] = np.nan
        # Interpolation linéaire
        idx = np.arange(len(signal_clean))
        valides = ~np.isnan(signal_clean)
        signal_clean = np.interp(idx, idx[valides], signal_clean[valides])
    else:
...
ax.plot(times[zoom_mask], signal[zoom_mask], color="steelblue", linewidth=0.8, label="Signal")
# Marquer les artefacts en rouge si détectés
if n_artefacts > 0:
    artefact_zoom = artefact_mask[zoom_mask]
    times_zoom = times[zoom_mask]
    signal_zoom = signal[zoom_mask]
    ax.fill_between(times_zoom, ax.get_ylim()[0], signal_zoom.max(),
                    where=artefact_zoom, color="crimson", alpha=0.3, label="Artefacts")
...
# ─────────────────────────────────────────────────────────────────────────────
# ÉTAPE 4 — Décomposition phasic / tonic (une seule fois → session_state)
# ─────────────────────────────────────────────────────────────────────────────
...
if st.button("Décomposer le signal EDA"):
    try:
        # Utiliser le signal corrigé si l'option est activée
        signal_pour_decomp = signal_clean if n_artefacts > 0 else signal
        signals_nk = analyze_eda(signal_pour_decomp, sampling_rate=int(sfreq))
        tonic  = signals_nk["EDA_Tonic"].values
        phasic = signals_nk["EDA_Phasic"].values
        st.session_state["eda_results"] = {
            "signal":       signal,
            "signal_clean": signal_pour_decomp,
            "times":        times,
            "sfreq":        sfreq,
            "tonic":        tonic,
            "phasic":       phasic,
...
# ─────────────────────────────────────────────────────────────────────────────
# ÉTAPE 5 — Détection des pics (interactive, sans relancer la décomposition)
# ─────────────────────────────────────────────────────────────────────────────
